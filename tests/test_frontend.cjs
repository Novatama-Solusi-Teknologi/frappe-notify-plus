...
function boot(saved = new Map(), blocked = false) {
    ...
    const frappe = {session: {user: 'test@example.com'}, realtime: {on() {}}, set_route: (...args) => routes.push(args)};
    vm.runInNewContext(source, {frappe, document, window: {AudioContext, addEventListener: (event, fn) => windowHandlers[event] = fn},
        ...
        __: x => x, $: fn => fn(), setTimeout, clearTimeout, URL, location: {origin: 'https://erp.test'}});
    return {frappe, elements, contexts, handlers, windowHandlers, routes, control: elements.find(x => x.className === 'np-audio-control')};
}
